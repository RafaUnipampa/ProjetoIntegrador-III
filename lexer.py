...
# -----------------------------
tokens = (
    'IF', 'DEFUN',  # Palavras-chave (apenas as usadas na gramática LISP)
    'EQ', 'NE', 'LT', 'LE', 'GT', 'GE',  # Operadores de comparação (simbólicos e textuais)
...
# Comentários de bloco serão ignorados \/
...
t_GE = r'>='
t_NE = r'!='
t_EQ = r'==|='   # aceita '==' e '=' para suportar (= a b)
t_LT = r'<'
t_GT = r'>'
...
def t_error(t):  # Mostrar a linha problemática, o lineno
    col = find_column(t.lexer.lexdata, t)
    print(f'ERROR 404! -> Caracter inesperado "{t.value[0]}" na linha {t.lexer.lineno}, coluna {col}')
    t.lexer.skip(1)

# Utilitário para coluna
def find_column(text, token):
    last_cr = text.rfind('\n', 0, token.lexpos)  # acha o ultimo quebra de linha antes do token
    if last_cr < 0:  # utilitário: converte posição absoluta (lexpos) em número de coluna na linha
        last_cr = -1
    return token.lexpos - last_cr

# Função principal: tokeniza e retorna lista de tokens (útil para testes isolados)
def scan(text):
    lexer = lex.lex()
    lexer.input(text)
    out = []
